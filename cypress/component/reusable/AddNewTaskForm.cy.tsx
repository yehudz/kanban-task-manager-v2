...
describe('Add new task form', ()=> {
  ...
  it('should have form title', ()=> {
    cy.get('[data-testid=task-form-title]').should('exist')
  })

  it('should have a title input', ()=> {
    cy.get('[data-testid=task-form-title-input]').should('exist').type('Test task')
  })
  it('should have a description text-area', ()=> {
    let testDescription = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.'
    cy.get('[data-testid=task-form-description-textarea]').should('exist').type(testDescription)
  })
  ...
  it('should have a subtasks container and placeholder inputs', ()=> {
    ...
    cy.get('[data-testid=task-form-subtask-input]').should('exist')
    cy.get('[data-testid=task-form-subtask-input]').should('exist')
    cy.get('[data-testid="primaryButton"]').click({multiple: true})
    cy.get('[data-testid=task-form-subtasks-container]').children().should('have.length', 3)
  })

  it('should have status drop down', ()=> {
    cy.get('[data-testid=task-form-status-select]').should('exist')
  })
